...
class CF(nn.Module):
    def __init__(self, c1, d):
        super(CF, self).__init__()
        self.cv1 = Conv(c1, c1, 1, g=c1)
        self.cv2 = Conv(c1, c1, 1, g=c1)
        self.cv3 = Conv(c1, c1, 1, g=c1)

    def forward(self, x):
        return self.cv3(self.cv1(x[0]) + self.cv2(x[1]))
##### end of TSCODE #####
...
class Convmix(nn.Module):  # x是输入，让x先过个分支 Resnet(卷积+池化+BN),之后再和原本的x相加。最后得到的和过一个1×1卷积,目的是调整通道
